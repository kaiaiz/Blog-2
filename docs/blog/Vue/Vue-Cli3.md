...
是否使用 history 路由模式（No）：这里看具体情况，因为 history 路由模式是要和后端配合的
...
同理 npm run dev 也只会运行 dev 模式下的 BASE_API ，这样就能和 axios 配置不同环境下的后端地址

### 拆分第三方代码段

全局引入 element-ui ，引入前的大小
...
由于代码没有拆分，可以看到 chunk-vendors 这个 js 相当的大
...
可以看到 element 的 js 和 css 被拆分了出来，还拆分了公共库，如 vue/vuex/axios 这类第三方依赖包
...
app 可以用来存储应用的状态，比如 element ui 中的全局组件 size；
...
新建一个文件夹 vue-cli3，名字随便取，但是要记住，后面要用到
...
不过使用 CDN 要慎重，如果项目部署在局域网或者网络很不好的地方，使用 CDN 就踩坑了
...
其实不使用 CompressionWebpackPlugin 插件，直接在 nginx 中开启 gzip 也是可以的，但是会消耗服务器的 cpu 性能

毕竟服务器没有我们自己的电脑强，所以就通过插件在本地完成压缩，这样能替服务器分担一点
